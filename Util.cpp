 ...
 * ssize_t is able to represent the number -1, which is returned by several system calls and library functions
 * as a way to indicate error.
 *
*/
...
void handle_for_sigpipe()//该函数的作用？？
{
    struct sigaction sa;
...
int socket_bind_listen(int port)
{
    ...
    if((listen_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
        return -1;
    // 避免bind时"Address already in use"错误
    int optval = 1;
