...
const __uint32_t DEFAULT_EVENT = EPOLLIN | EPOLLET | EPOLLNESHOT;
...
:   loop_(loop),
    ...
    keepAlive_(false)
{
    channel_->setReadhandler(bind(&HttpData::handleRead, this));//bind返回可调用对象
...
void HttpData::reset()
{
    ...
    hState_ = H_START;
    headers_.clear();
    if(timer_.lock())//lock()，creates a shared_ptr that manages the referenced object
    {
...
AnalysisState HttpData::analysisRequest()
{
    ...
    else if(method_ == METHOD_GET || method_ == METHOD_HEAD)
    {
        ...
        if(fileName_ == "favicon.ico")
        {
            ...
            header += "Server: Webserver created by Feng\r\n";
            header += "\r\n";
            outBuffer_ += header;
            outBuffer_ += header;
...
void HttpData::handleError(int fd, int err_num, std::string short_msg)
{
    ...
    body_buff += "<html><title>Something is Wrong ~</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += to_string(err_num) + short_msg;
    body_buff += "<hr><em> Webserver created by Feng</em>\n</body><html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size())+ "\r\n";
    header_buff += "Server: Webserver created by Feng\r\n";
    header_buff += "\r\n";
